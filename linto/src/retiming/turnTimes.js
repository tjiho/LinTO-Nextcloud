// Ported from linto-studio/studio-api components/EditorHandler/utils/turnTimes.js
// (CommonJS -> ESM only, logic untouched).

/** First DEFINED time of a word list — robust to sparse/absent timings. */
export function firstDefinedTime(words, field) {
  for (const w of words) if (w[field] != null) return w[field]
  return undefined
}

export function lastDefinedTime(words, field) {
 ...
 *  never writing null/undefined. */
export function assignTurnTimes(turn, stime, etime) {
  delete turn.stime
  delete turn.etime
